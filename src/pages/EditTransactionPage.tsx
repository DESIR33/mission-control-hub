    ...
    if (editId && transactions.length > 0) {
      ...
      if (transaction) {
        setFormData({
          ...
          approximatePayoutDate: transaction.approximatePayoutDate
            ? transaction.approximatePayoutDate.split("T")[0]
            : "",
  ...
  const updateTransaction = useMutation({
    mutationFn: async () => {
      ...
      const response = await fetch(
        `/api/affiliate-programs/${id}/transactions/${editId}`,
        {
          method: "PATCH",
          headers: {
          ...
          body: JSON.stringify({
            amount: parseFloat(formData.commission.toString()),
            transactionDate: formData.transactionDate,
            approximatePayoutDate:
              formData.approximatePayoutDate || null,
  ...
  if (!editId) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        No transaction specified to edit.
      </div>
    );
  }
