...
function AiBridgeContent() {
  ...
  const [localEdits, setLocalEdits] = useState<
    Record<string, { proposed_changes: Record<string, unknown>; summary: string }>
  >({});
  ...
  const effectiveProposals = useMemo(
    () =>
      proposals.map((p) => ({
        ...
        ...(localEdits[p.id]
          ? {
              proposed_changes: localEdits[p.id].proposed_changes,
              summary: localEdits[p.id].summary,
            }
          : {}),
        status: localStatuses[p.id] ?? p.status,
        reviewed_at:
          localStatuses[p.id] && localStatuses[p.id] !== p.status
  ...
  const handleApprove = (id: string) => {
    // Optimistic local update (works for both mock and DB data)
      ...
      { id, status: "approved" },
      {
        onSuccess: () => {
          toast({
            ...
            description: "The AI proposal has been approved and changes will be applied.",
          });
        },
        onError: () => {
          // For mock data, the DB call may fail but local state already reflects the action
          toast({
            title: "Proposal approved",
            description: "Approved locally (demo mode).",
          });
        },
      }
    );
      ...
      { id, status: "rejected" },
      {
        onSuccess: () => {
          toast({
            ...
            description: "The AI proposal has been rejected.",
          });
        },
        onError: () => {
          toast({
            title: "Proposal rejected",
            description: "Rejected locally (demo mode).",
          });
        },
      }
    );
  ...
  const handleEdit = (proposal: AiProposal) => {
    ...
    updateProposal.mutate(
      { id, proposed_changes: changes, summary },
      {
        onSuccess: () => {
          ...
          toast({
            title: "Proposal edited & approved",
            description: "Changes saved and proposal approved.",
          });
        },
        onError: () => {
          toast({
  ...
  const pendingCount = effectiveProposals.filter(
    (p) => p.status === "pending"
  ).length;
    ...
    <div className="p-4 sm:p-6 lg:p-8 gradient-mesh min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          Approve, reject, or edit before changes are applied.
        </p>
      </div>
          ...
          <Tabs defaultValue="pending" className="mt-6">
            <TabsList>
              <TabsTrigger value="all">
                All ({effectiveProposals.length})
              </TabsTrigger>
              <TabsTrigger value="pending">
                Pending ({pendingCount})
              </TabsTrigger>
              <TabsTrigger value="reviewed">
                Reviewed ({effectiveProposals.length - pendingCount})
              </TabsTrigger>
            </TabsList>
              ...
              <TabsContent key={tab} value={tab} className="mt-4 space-y-3">
                {(() => {
                  const filtered = filterBySearch(
                    filterByTab(tab, effectiveProposals)
                  );
                    ...
                    <>
                      ...
                      {filtered.map((proposal) => (
                        <ProposalCard
                          ...
                          onEdit={handleEdit}
                          isActioning={updateStatus.isPending}
                        />
                      ))}
                    </>
                  );
