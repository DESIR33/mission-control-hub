...
const TERM_CATEGORY_META: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  general: { label: "Campaign Terms", icon: Scale, color: "text-muted-foreground" },
  affiliate: { label: "Affiliate Requirements", icon: Link2, color: "text-emerald-400" },
};
...
function EditableRow({
  ...
  const handleCancel = () => {
    setName(item.name); setDescription(item.description ?? ""); setPrice(String(item.price)); setEditing(false);
  };
...
function EditableTermRow({
  term, onSave, onDelete,
}: {
  term: RateCardTerm;
  onSave: (updates: Partial<RateCardTerm> & { id: string }) => void;
  onDelete: (id: string) => void;
}) {
  const [editing, setEditing] = useState(false);
  const [content, setContent] = useState(term.content);

  const handleSave = () => { onSave({ id: term.id, content }); setEditing(false); };
  const handleCancel = () => { setContent(term.content); setEditing(false); };

  if (editing) {
    return (
      <li className="flex items-start gap-2 bg-muted/20 rounded-lg p-2">
        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          className="flex-1 bg-background border border-input rounded px-2 py-1.5 text-xs text-foreground resize-none min-h-[48px]"
        />
        <div className="flex flex-col gap-1">
          <Button size="icon" variant="ghost" className="h-6 w-6" onClick={handleSave}><Check className="w-3 h-3 text-green-400" /></Button>
          <Button size="icon" variant="ghost" className="h-6 w-6" onClick={handleCancel}><X className="w-3 h-3 text-muted-foreground" /></Button>
        </div>
      </li>
    );
  }
  ...
  return (
    <li className="flex items-start gap-2 group">
      ...
      <p className="text-xs text-muted-foreground flex-1 leading-relaxed">{term.content}</p>
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => setEditing(true)}><Pencil className="w-2.5 h-2.5" /></Button>
        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => onDelete(term.id)}><Trash2 className="w-2.5 h-2.5 text-destructive" /></Button>
      </div>
    </li>
  );
}
...
function AddItemForm({ onAdd }: { onAdd: (item: Omit<RateCardItem, "id" | "workspace_id">) => void }) {
  ...
  const handleSubmit = () => {
    ...
    setName(""); setDescription(""); setPrice(""); setCategory("video"); setOpen(false);
  };

  if (!open) {
    return (
      <Button variant="ghost" size="sm" className="text-xs text-muted-foreground" onClick={() => setOpen(true)}>
        <Plus className="w-3 h-3 mr-1" /> Add pricing item
      </Button>
    );
  }
...
/* ── Add Term Form ── */
function AddTermForm({ onAdd }: { onAdd: (term: Omit<RateCardTerm, "id" | "workspace_id">) => void }) {
  const [open, setOpen] = useState(false);
  ...
  const handleSubmit = () => {
    ...
    onAdd({ content, category, sort_order: 99, is_active: true });
    setContent(""); setCategory("general"); setOpen(false);
  };
...
export function RateCardCalculator() {
  const {
    items, terms, isLoading, needsSeed, seedDefaults,
  ...
  if (needsSeed) {
    return (
      <div className="rounded-xl border border-border bg-card p-8 text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">No rate card set up yet. Load default pricing &amp; terms?</p>
  ...
  const topRate = items.filter((i) => i.category === "video").reduce((s, i) => Math.max(s, i.price), 0);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-border bg-card p-3">
          ...
          <p className="text-lg font-bold font-mono text-foreground">${topRate.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border bg-muted/30 flex items-center gap-2">
          ...
          <h3 className="text-xs font-semibold text-foreground uppercase tracking-wider">Campaign Terms &amp; Conditions</h3>
        </div>
        <div className="p-4 space-y-4">
          {(["general", "affiliate"] as const).map((cat) => {
            const catTerms = groupedTerms[cat];
            if (!catTerms?.length) return null;
            const meta = TERM_CATEGORY_META[cat];
            return (
              <div key={cat}>
          ...
          <AddTermForm onAdd={(term) => addTerm.mutate(term)} />
        </div>
      </div>
    </div>
  );
