...
function SummaryStats({ collaborations }: { collaborations: Collaboration[] }) {
  const stats = useMemo(() => {
    const byCounts: Record<CollabStatus, number> = {
      prospect: 0, contacted: 0, negotiating: 0, confirmed: 0, published: 0, declined: 0,
    };
    let totalExpected = 0;
    let totalActual = 0;
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Published Collabs</p>
          <p className="text-lg font-bold text-foreground">{published.length}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Total Sub Gain</p>
          <p className="text-lg font-bold text-emerald-400">{formatSubGain(totalActual)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Avg Sub Gain</p>
          <p className="text-lg font-bold text-foreground">{formatSubGain(Math.round(avgGain))}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Actual vs Expected</p>
          <p className="text-lg font-bold text-foreground">{roiPercent}%</p>
        </div>
      </div>
      <div className="space-y-2">
        {published.map((c) => {
          const pct = c.expected_sub_gain && c.expected_sub_gain > 0
            ? ((c.actual_sub_gain ?? 0) / c.expected_sub_gain) * 100
            : null;
          return (
            <div key={c.id} className="flex items-center justify-between text-xs rounded-md bg-secondary/50 px-3 py-2">
              <span className="font-medium text-foreground">{c.creator_name}</span>
              <div className="flex items-center gap-3">
                <span className="text-muted-foreground">Expected: {formatSubGain(c.expected_sub_gain)}</span>
                <span className="text-emerald-400 font-medium">Actual: {formatSubGain(c.actual_sub_gain)}</span>
                {pct !== null && (
                  <Badge variant="outline" className={pct >= 100 ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" : "bg-amber-500/15 text-amber-400 border-amber-500/30"}>
                    {pct.toFixed(0)}%
                  </Badge>
                )}
              </div>
...
function CollabCard({
  collab, onSelect, onMoveForward, onMoveBack,
}: {
  collab: Collaboration; onSelect: () => void;
  onMoveForward: (() => void) | null; onMoveBack: (() => void) | null;
}) {
  return (
    <div className="rounded-lg border border-border bg-card p-3 space-y-2 cursor-pointer hover:border-primary/50 transition-colors" onClick={onSelect}>
      <div className="flex items-start justify-between gap-1">
        <h4 className="text-sm font-semibold text-foreground truncate">{collab.creator_name}</h4>
        {collab.collab_type && (
          <Badge variant="outline" className="text-xs shrink-0">{COLLAB_TYPE_LABELS[collab.collab_type]}</Badge>
        )}
      </div>
      <div className="space-y-1 text-xs text-muted-foreground">
        {collab.subscriber_count !== null && (
          <div className="flex items-center gap-1"><Users className="w-3 h-3" /><span>{formatSubscribers(collab.subscriber_count)} subs</span></div>
        )}
        {collab.niche && (
          <div className="flex items-center gap-1"><Eye className="w-3 h-3" /><span className="truncate">{collab.niche}</span></div>
        )}
        {collab.scheduled_date && (
          <div className="flex items-center gap-1"><Calendar className="w-3 h-3" /><span>{collab.scheduled_date}</span></div>
        )}
        {(collab.expected_sub_gain !== null || collab.actual_sub_gain !== null) && (
          <div className="flex items-center gap-1">
            <TrendingUp className="w-3 h-3" />
            <span>Exp: {formatSubGain(collab.expected_sub_gain)}{collab.actual_sub_gain !== null && <> / Act: {formatSubGain(collab.actual_sub_gain)}</>}</span>
          </div>
        )}
      </div>
      <div className="flex items-center gap-1 pt-1">
        {onMoveBack && (
          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); onMoveBack(); }} title="Move to previous stage" aria-label="Move to previous stage">
            <ArrowLeft className="w-3 h-3" />
          </Button>
        )}
        {onMoveForward && (
          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); onMoveForward(); }} title="Move to next stage" aria-label="Move to next stage">
            <ArrowRight className="w-3 h-3" />
          </Button>
        )}
        {collab.channel_url && (
          <Button variant="ghost" size="icon" className="h-6 w-6 ml-auto" onClick={(e) => { e.stopPropagation(); window.open(collab.channel_url!, "_blank"); }} title="Open channel" aria-label="Open channel">
...
function KanbanColumn({ status, collaborations, onSelect, onMove }: {
  status: CollabStatus; collaborations: Collaboration[];
  onSelect: (c: Collaboration) => void; onMove: (id: string, newStatus: CollabStatus) => void;
}) {
  const statusIdx = STATUSES.indexOf(status);
  return (
    <div className="flex flex-col min-w-[260px] max-w-[300px] flex-1">
      <div className="flex items-center gap-2 mb-3 px-1">
        <Badge variant="outline" className={`${STATUS_COLORS[status]} text-xs`}>{STATUS_LABELS[status]}</Badge>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto max-h-[60vh] pr-1">
        {collaborations.map((c) => {
          ...
          return (
            <CollabCard key={c.id} collab={c} onSelect={() => onSelect(c)}
              onMoveForward={canForward ? () => onMove(c.id, STATUSES[statusIdx + 1]) : null}
              onMoveBack={canBack ? () => onMove(c.id, STATUSES[statusIdx - 1]) : null}
            />
          );
        })}
...
function CollabFormFields({ form, onChange, showStatus }: {
  form: CollabFormData; onChange: (f: CollabFormData) => void; showStatus?: boolean;
}) {
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="creator_name">Creator Name *</Label>
        <Input id="creator_name" value={form.creator_name} onChange={(e) => onChange({ ...form, creator_name: e.target.value })} placeholder="Creator name" className="bg-secondary border-border" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <Label htmlFor="channel_url">Channel URL</Label>
          <Input id="channel_url" value={form.channel_url} onChange={(e) => onChange({ ...form, channel_url: e.target.value })} placeholder="https://youtube.com/@..." className="bg-secondary border-border" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="subscriber_count">Subscriber Count</Label>
          <Input id="subscriber_count" type="number" value={form.subscriber_count} onChange={(e) => onChange({ ...form, subscriber_count: e.target.value })} placeholder="100000" className="bg-secondary border-border" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <Label htmlFor="niche">Niche</Label>
          <Input id="niche" value={form.niche} onChange={(e) => onChange({ ...form, niche: e.target.value })} placeholder="Tech, Gaming, etc." className="bg-secondary border-border" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="collab_type">Collab Type</Label>
          <Select value={form.collab_type || undefined} onValueChange={(v) => onChange({ ...form, collab_type: v as CollabType })}>
            <SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="Select type" /></SelectTrigger>
            <SelectContent>
              {COLLAB_TYPES.map((t) => (<SelectItem key={t} value={t}>{COLLAB_TYPE_LABELS[t]}</SelectItem>))}
            </SelectContent>
          </Select>
        </div>
      </div>
      {showStatus && (
        <div className="space-y-2">
          ...
          <Select value={form.status} onValueChange={(v) => onChange({ ...form, status: v as CollabStatus })}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          ...
          <Input id="expected_sub_gain" type="number" value={form.expected_sub_gain} onChange={(e) => onChange({ ...form, expected_sub_gain: e.target.value })} placeholder="5000" className="bg-secondary border-border" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="actual_sub_gain">Actual Sub Gain</Label>
          <Input id="actual_sub_gain" type="number" value={form.actual_sub_gain} onChange={(e) => onChange({ ...form, actual_sub_gain: e.target.value })} placeholder="4200" className="bg-secondary border-border" />
        </div>
      </div>
      <div className="space-y-2">
        <Label htmlFor="scheduled_date">Scheduled Date</Label>
        <Input id="scheduled_date" type="date" value={form.scheduled_date} onChange={(e) => onChange({ ...form, scheduled_date: e.target.value })} className="bg-secondary border-border" />
      </div>
      <div className="space-y-2">
        <Label htmlFor="notes">Notes</Label>
        <Textarea id="notes" value={form.notes} onChange={(e) => onChange({ ...form, notes: e.target.value })} placeholder="Additional notes..." className="bg-secondary border-border min-h-[80px]" />
      </div>
    </div>
  );
}
...
export function CollaborationsContent() {
  ...
  const grouped = useMemo(() => {
    const map: Record<CollabStatus, Collaboration[]> = {
      prospect: [], contacted: [], negotiating: [], confirmed: [], published: [], declined: [],
    };
    for (const c of collaborations) {
      map[c.status].push(c);
    }
  ...
  const handleAdd = async () => {
    if (!addForm.creator_name.trim()) { toast.error("Creator name is required"); return; }
    try {
  ...
  const handleUpdate = async () => {
    ...
    if (!editForm.creator_name.trim()) { toast.error("Creator name is required"); return; }
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {STATUSES.map((status) => (
          <KanbanColumn key={status} status={status} collaborations={grouped[status]} onSelect={handleSelectCollab} onMove={handleMove} />
        ))}
      </div>
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Add a new creator collaboration to your pipeline.</DialogDescription>
          </DialogHeader>
          <CollabFormFields form={addForm} onChange={setAddForm} />
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleAdd} disabled={createCollab.isPending}>
              {createCollab.isPending ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Adding...</>) : "Add Collaboration"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="overflow-y-auto sm:max-w-lg">
          <SheetHeader>
            ...
            <SheetDescription>Edit collaboration details or change its status.</SheetDescription>
          </SheetHeader>
          <div className="mt-6">
            <CollabFormFields form={editForm} onChange={setEditForm} showStatus />
          </div>
          <SheetFooter className="mt-6 flex gap-2 sm:justify-between">
            <Button variant="destructive" size="sm" onClick={handleDelete} disabled={deleteCollab.isPending}>
              {deleteCollab.isPending ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Trash2 className="w-4 h-4 mr-1" />}
              Delete
            </Button>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setSheetOpen(false)}>Cancel</Button>
              <Button onClick={handleUpdate} disabled={updateCollab.isPending}>
                {updateCollab.isPending ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Saving...</>) : "Save Changes"}
              </Button>
            </div>
          </SheetFooter>
        </SheetContent>
