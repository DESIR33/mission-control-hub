...
const REVENUE_DATASETS = ["youtubeMonetisation", "youtubeAnalytics"];

export function RevenueHubSection() {
  const { getStatus, canRefreshNow } = useDatasetSyncStatus(REVENUE_DATASETS);
  const refreshMutation = useManualDatasetRefresh();

  const monetisationStatus = getStatus("youtubeMonetisation");
  const canRefresh = canRefreshNow("youtubeMonetisation");
        ...
        <DataFreshnessBadge
          ...
          onRefresh={() =>
            refreshMutation.mutate({
              datasetKey: "youtubeMonetisation",
              edgeFunctionName: "youtube-analytics-sync",
            })
          }
        />
      </div>
